...
a5.dp.info = {
  engine: `${a5.Engine.version || 'development'} (recommended: buckminster-41.0.6)`,
  root: 'core-41.2.4'
};
...
_.extend(a5.dp.config, {
  ...
  languageAvailable: ['en', 'de', 'es', 'fr', 'it', 'nl', 'zh', 'ar', 'pt', 'uk', 'pt_BR', 'de-INFORMAL'],  // String Array   Multilanguage support. Available translations. (example: ['en-US', 'es-ES'] - default: empty [])
...
(function(core, LearningObject, Progress, jQuery) {
  ...
  core.Media = (function() {
    ...
    var locators = {
      ...
      miniPlayerDraggables: '.dragAudioPlayer',
...
(function(core, LearningObject, jQuery) {
  ...
  core.PoolHelper = (function() {
     ...
     */
     PoolHelper.prototype._carouselControls = function () {
      const offsetLeft = this.elements.scrollLeft();
  ...
  a5.callbacks.interaction.bind('showed', function (interaction) {
    ...
    if(isVerticalPool && interaction.identifier == "Order:Sort:Sorting") new core.PoolHelper(interaction);
  }, this);
...
$(function() {
  ...
  function checkHasImage($item) {
    ...
    if ($image.length != 0 && $item.find('.content').text().length != 0) {
      $item.find('.content').addClass('draggable__content--hasImage');

      if ($item.text().length != $item.find('.image-div').text().length ) {
        $item.find('.content').addClass('draggable__content--hasImageWithText');
        $item.addClass('os-sequence-element--imageWithText');
      }
    }
  }
...
_.extend(a5.dp.config, {
  ...
  languageAvailable: ['en', 'es', 'pt_BR'],     // String Array   Multilanguage support. Available translations. (example: ['en-US', 'es-ES'] - default: empty [])
  ...
  ICTextgapActive: false,                       // Boolean        Focus the first gaptext in the interaction (default: true). Not supported by vertical DPs.
...
_.extend(a5.core.config, {
  ...
  loToolbarGap: 40,                             // Integer (px)   Gap bettween the activity header and the content wrap. Needs to factor in your annotation tool button width it positioned out of the flow (default: 20)
...
(function() {
  ...
  var builderOptions = function() {
    ...
    _.each(DROPDOWNS, function(key) {
      ...
      if (value) {
        document.documentElement.classList.add(`theme${key}--${value.replace(/\s+/g, '')}`);
      }
    });
...
// Extra content for End Results screen in Adaptive

$(function() {
  a5.hooks.interactionShowed.add(function(interaction) {
    if (interaction.isEndResultScreen() && document.getElementById('er-extra') != null && document.getElementById('er-extra-content') != null) {
      document.getElementById('er-extra').style.display = 'block';

      var sectionCount = document.getElementsByClassName('ergrammpoint').length,
          vidCount = sectionCount * 2,
          notAttempted = parseInt(document.getElementById('notattval').innerText),
          skippables = sectionCount * 3,
          supportNeeded = skippables + 1 + vidCount - notAttempted, // 1 Added for the Submit screen
          supportvalHTML = `<p>Number of support activities used: <span class="supportvaltext">${supportNeeded}</span></p>`,
          supportMessageHTML =  '';

      if (supportNeeded === 0) {
        supportMessageHTML = '<p><strong>Excellent!</strong> You didn&#39;t need any support in this unit.</p>';
      } else if (0 < supportNeeded && supportNeeded <= sectionCount) {
        supportMessageHTML = '<p><strong>Good effort.</strong> You only needed to use a few support activities in this unit.</p>';
      } else if (sectionCount < supportNeeded && supportNeeded < skippables) {
        supportMessageHTML = '<p>You needed quite a lot of support in this unit.</p>';
      } else if (supportNeeded == skippables) {
        supportMessageHTML = '<p>You used all of the support activities in this unit.</p>';
      }

      document.getElementById('er-extra-content').innerHTML = supportvalHTML + supportMessageHTML;
    }
  });
});
...
_.extend(a5.dp.info, {
  child2: 'richmond-rdp-preschool-5.1.2'
});
...
_.extend(a5.core.config, {
  ...
  attachmentBadge: true,                            // Boolean        Enables the attachment count badge

  // Pagination

  paginationRange: 2,                               // Integer        Number of pages that will get a --prev & --next class that can be used to limit the number of visible items or style them in a different way (default: false to disable it)
  paginationProgressBar: false                      // Boolean        Enables the pagination progress bar
});
...
_.extend(a5.dp.richmondRDPconfig, {
  scrollWatcher: false,
  titleSegmentation: false
});
...
// Sound effects in SubmitInfo & SubmitFinish dialogs

(function() {
  let submitInfoSound = new Audio();

  a5.callbacks.dialogs.get('a5.view.dialog.SubmitInfo').bind('showed', function() {
    const scores = a5.mainBuilder.getScores();

    if (scores.total === scores.correct) {
      submitInfoSound.src = A5.SKIN_ROOT + 'audios/great-job.mp3';
    } else {
      if (scores.correct === 0) {
        submitInfoSound.src = A5.SKIN_ROOT + 'audios/keep-practicing.mp3';
      } else {
        submitInfoSound.src = A5.SKIN_ROOT + 'audios/good-work.mp3';
      }
    }

    submitInfoSound.play();
  });

  a5.callbacks.dialogs.get('a5.view.dialog.SubmitFinish').bind('showed', function() {
    submitInfoSound.src = A5.SKIN_ROOT + 'audios/great-job.mp3';
    submitInfoSound.play();
  });
})();
